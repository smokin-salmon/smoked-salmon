...
def _check_flac_integrity(path):
    try:
        result = subprocess.check_output(["flac", "-wt", path], text=True, encoding="utf-8", stderr=subprocess.STDOUT)
        important_lines = []
...
def _check_mp3_integrity(path):
    try:
        result = subprocess.check_output(["mp3val", path], text=True, encoding="utf-8")
        important_lines = []
        for line in result.split("\n"):
