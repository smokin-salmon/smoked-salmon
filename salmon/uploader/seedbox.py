...
class RcloneUploader(Uploader):
    ...
    def add_to_downloader(self, remote_folder, path, type):
        ...
        try:
            self.client.add_to_downloader(shell_path, torrent, is_paused=False, label="smoked-salmon")
    ...
    def add_to_downloader(self, remote_folder, path, type):
        ...
        try:
            self.client.add_to_downloader(
                cfg.directory.download_directory, torrent, is_paused=False, label="smoked-salmon"
            )
...
class UploadManager:
        ...
        for seedbox in cfg.seedbox:
            try:
                ...
                self.uploaders.append(
                    {"uploader": uploader, "directory": seedbox.directory, "flac_only": seedbox.flac_only}
                )
    ...
    def execute_upload(self):
        ...
        for i, task in enumerate(self.tasks, 1):
            uploader, remote_directory, local_directory, task_type = task
            try:
                click.secho(
